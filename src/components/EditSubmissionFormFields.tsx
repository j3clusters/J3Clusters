...
export type EditSubmissionDefaults = {
  id: string;
  ownerName: string;
  ownerEmail: string;
  ownerPhone: string;
...
type EditSubmissionFormFieldsProps = {
  formId: string;
  formAction: (formData: FormData) => void | Promise<void>;
  defaults: EditSubmissionDefaults;
  maxImages?: number;
};

export function EditSubmissionFormFields({
  formId,
  formAction,
  defaults,
  maxImages = 10,
  ...
  const requiresBalconyAndParking = requiresRoomDetails;
  ...
  const initialImages =
    defaults.imageUrls.length > 0 ? defaults.imageUrls : ([] as string[]);
